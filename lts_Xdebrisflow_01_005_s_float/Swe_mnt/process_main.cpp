...
void Process_main::staggerResult(Phase& solid)
{
    for (int k=0;k<3;k++)
    {
        float dx=Data::instance()->get_Dx();
        float dy=Data::instance()->get_Dy();
        for(int i=1;i<solid.get_PhaseSize().getRow_Size()-1;i++)
        {
            for (int j=1;j<solid.get_PhaseSize().getCol_Size()-1;j++)
            {
        ...
        for(int i=1;i<solid.get_PhaseSize().getRow_Size()-1;i++)
        {
            for (int j=1;j<solid.get_PhaseSize().getCol_Size()-1;j++)
            {
                ...
                (*solid._U[k])[i][j]=(*_NUstagRes[k])[i][j]
                        +_dt*(*term._sourceTerm[k])[i][j];
                if((*solid._U[0])[i][j]<=0)
                {(*solid._U[0])[i][j]=Data::instance()->get_Eps();
                 std::cout<<" Warning_1 ";}
            }
        }
    }
...
{
    _crt+=_dt;
    float maxVelo=max(_maxSpeed->_x,_maxSpeed->_y);
...
{
    Matrix_data re;
    float alfa=Data::instance()->get_Alfa();
    for (int i=0;i<re.getRow();i++)
    {
        for (int j=0;j<re.getCol();j++)
        {
            float mid_1=alfa/2*(sign(input_1._mat[i][j])+sign(input_2._mat[i][j]))
                    *min(abs2(input_1._mat[i][j]),abs2(input_2._mat[i][j]));
            float mid_2=(input_1._mat[i][j]+input_2._mat[i][j])/2;
            re._mat[i][j]=0.5*(sign(mid_1)+sign(mid_2))*min(abs2(mid_1),abs2(mid_2));
        }
    }
    return re;
}
float MidPara::Xmic(float input_1,float input_2)
{
    float alfa=Data::instance()->get_Alfa();
    float mid_1=alfa/2*(sign(input_1)+sign(input_2))
            *min(abs2(input_1),abs2(input_2));
    float mid_2=(input_1+input_2)/2;
