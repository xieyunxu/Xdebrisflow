...
{   //  #(HEAD) Keyword    Values(Defaults)           Tips
    ...
    if(vec[0]==kwSource[2]){
        ...
        if(vec[1]!=" "&&vec.size()>=2)
        {
            Data::instance()->set_TotalTime(FO::stringToDouble(vec[1]));
    ...
    if (vec[0]==kwSource[3]){
        ...
        if(vec[1]!=" "&&vec.size()>=2)
        {
            Data::instance()->set_IntervalTime(FO::stringToDouble(vec[1]));
        ...
        if(vec[1]!=" "&&vec.size()>=2)
        {
            ...
            if(FO::stringToInt(vec[1])==2){
                ...
                if(vec.size()>=7&&vec.size()<=8){
                    if(FO::stringToInt(vec[4])>0){Data::instance()->set_SourceDepth(FO::stringToDouble(vec[3]));}
